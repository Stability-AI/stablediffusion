...
def predict(input_image, prompt, steps, num_samples, scale, seed, eta, noise_level):
    ...
    noise_level = (
        torch.Tensor(num_samples * [noise_level]).to(sampler.model.device).long()
    )
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                steps = gr.Slider(
                    label="DDIM Steps", minimum=2, maximum=200, value=75, step=1
                )
                scale = gr.Slider(
                ...
                eta = gr.Number(label="eta (DDIM)", value=0.0, min=0.0, max=1.0)
                noise_level = None
    ...
    run_button.click(
        fn=predict,
        inputs=[input_image, prompt, steps, num_samples, scale, seed, eta, noise_level],
        outputs=[gallery],
    )
