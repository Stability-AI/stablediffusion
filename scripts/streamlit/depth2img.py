...
def make_batch_sd(image, txt, device, num_samples=1, model_type="dpt_hybrid"):
    ...
    batch["jpg"] = repeat(
        batch["jpg"].to(device=device), "1 ... -> n ...", n=num_samples
    )
    batch["midas_in"] = repeat(
        torch.from_numpy(batch["midas_in"][None, ...]).to(device=device),
        "1 ... -> n ...",
        n=num_samples,
    )
    return batch
...
def paint(
    ...
    with torch.no_grad(), torch.autocast("cuda"):
        ...
        z = model.get_first_stage_encoding(
            model.encode_first_stage(batch[model.first_stage_key])
...
def run():
    ...
    if image:
        ...
        def t_callback(t):
            t_progress.progress(min((t + 1) / t_enc, 1.0))

        assert 0.0 <= strength <= 1.0, "can only work with strength in [0.0, 1.0]"
        do_full_sample = strength == 1.0
        t_enc = min(int(strength * steps), steps - 1)
        sampler.make_schedule(steps, ddim_eta=0.0, verbose=True)
        if st.button("Sample"):
            result = paint(
                sampler=sampler,
                image=image,
